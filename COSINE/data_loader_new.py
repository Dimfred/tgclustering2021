...
class YelpProcessor(object):
    ...
    def get_examples(self, mode):
        ...
        return self._create_examples(
            self.read_data(os.path.join(self.args.data_dir, file_to_read), mode), mode
        )
...
class YoutubeProcessor(object):
    ...
    def __init__(self, args):
        ...
        if "agnews" in self.args.task:
            self.num_label = 4
        elif "roberta" in self.args.task:
            self.num_label = 42
    ...
    def _create_examples(self, data, set_type):
        ...
        for i, d in enumerate(data):
            guid = "%s-%s" % (set_type, i)
            text_a = d["text"]
            # TODO for our case always d["major"] cause no rule
            # hence label is in major
            label = (
                d["label"]
                if (self.rule == 0 and set_type != "unlabeled")
                else d["major"]
            )
            # print(text_a, label)
            # if i>10:
            ...
            examples.append(
                InputExample(guid=guid, text_a=text_a, label=label, true=true)
            )
...
def tokenize_with_2span(tokenizer, sent, span_a, span_b):
    ...
    if span_a[1] <= span_b[0]:
        ...
        return (
            ...
            len(_s0),
            len(_s0) + len(_wa),
            len(_s0) + len(_wa) + len(_s1),
            len(_s0) + len(_wa) + len(_s1) + len(_wb),
        )
    else:
        ...
        return (
            _s0 + _wb + _s1 + _wa + _s2,
            len(_s0) + len(_wb) + len(_s1),
            len(_s0) + len(_wb) + len(_s1) + len(_wa),
            len(_s0),
            len(_s0) + len(_wb),
        )


def convert_examples_to_features_re(
    examples,
    max_seq_len,
    tokenizer,
    cls_token="[CLS]",
    ...
    for (ex_index, example) in enumerate(examples[:]):
        ...
        tokens_a, key_a_start, key_a_end, key_b_start, key_b_end = tokenize_with_2span(
            tokenizer, example.text_a, example.span_a, example.span_b
        )
        keys = [0] * len(tokens_a)
        ...
        assert len(keys) == max_seq_len, "Error with input length {} vs {}".format(
            len(keys), max_seq_len
        )
        assert (
...
def convert_examples_to_features_wic(
    examples,
    max_seq_len,
    tokenizer,
    cls_token="[CLS]",
    ...
    for (ex_index, example) in enumerate(examples[:]):
        if ex_index % 5000 == 0:
            logger.info("Writing example %d of %d" % (ex_index, len(examples)))
        tokens_a, key_a_start, key_a_end = tokenize_with_span(
            tokenizer, example.text_a, example.span_a
        )
        tokens_b, key_b_start, key_b_end = tokenize_with_span(
            tokenizer, example.text_b, example.span_b
        )
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    if os.path.exists(cached_features_file) and args.auto_load:
        ...
        if args.task_type == "wic":
            (features,) = convert_examples_to_features_wic(
                examples,
                args.max_seq_len,
                tokenizer,
            ...
            features = convert_examples_to_features_re(
                examples,
                args.max_seq_len,
                tokenizer,
                add_sep_token=args.add_sep_token,
...
def load_and_cache_unlabeled_examples(args, tokenizer, mode, train_size=100):
    ...
    all_ids = torch.tensor(
        [_ + train_size for _, f in enumerate(features)], dtype=torch.long
    )
