...
def main(args):
    ...
    unlabeled_dataset, unlabeled_size = load_and_cache_unlabeled_examples(
        args, tokenizer, mode="unlabeled", train_size=train_size
    )
    ...
    trainer = Trainer(
        ...
        labelset=relation_labels,
        unlabeled=unlabeled_dataset,  # masked_train_dataset = masked_train_dataset, masked_dev_dataset = masked_dev_dataset, \
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--method", default="clean", type=str, help="which method to use"
    )
    parser.add_argument(
        "--task_type",
        default="re",
        type=str,
        help="the specific task in IE: [ tc | re | ner ]",
    )
    ...
    parser.add_argument(
        "--unlabel_file", default="unlabeled.tsv", type=str, help="Test file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--auto_load", default=1, type=int, help="Auto loading the model or not"
    )
    ...
    parser.add_argument(
        "--max_steps", default=-1, type=int, help="Training steps for initialization."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="whether soft label (0 for hard, 1 for soft)",
    )
    parser.add_argument(
        "--soft_label_weight", default=1.0, type=float, help="iters for pretrains"
    )
    parser.add_argument(
        "--self_training_eps", type=float, default=0.8, help="threshold for confidence"
    )
    parser.add_argument(
        "--self_training_power", type=float, default=2, help="power of pred score"
    )
    parser.add_argument(
        "--self_training_confreg", type=float, default=0, help="confidence smooth power"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--self_training_max_step",
        type=int,
        default=10000,
        help="the maximum step (usually after the first epoch) for self training",
    )
    parser.add_argument(
        "--distmetric",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="hard",
        help="pseudo label type. Choices = [hard, soft]",
    )
    parser.add_argument(
        "--self_training_update_period", type=int, default=100, help="update period"
    )
