...
class Trainer(object):
    def __init__(
        ...
        self.label_matrix = torch.eye(self.num_labels) * (
            self.w - self.k
        ) + self.k * torch.ones(self.num_labels)
    ...
    def calc_loss(
        self, input, target, loss, thresh=0.95, soft=True, conf="max", confreg=0.1
    ):
        ...
        if conf == "max":
            ...
            w = torch.FloatTensor([1 if x == True else 0 for x in weight > thresh]).to(
                self.device
            )
        elif conf == "entropy":
    ...
    def evaluate(self, mode, global_step=-1):
        ...
        if self.args.task_type == "re":
            write_prediction_re(
                self.args,
                os.path.join(self.args.eval_dir, "pred/proposed_answers.txt"),
                preds,
            )
            ...
            write_prediction_tc(
                ...
                os.path.join(
                    self.args.eval_dir,
                    "pred/pred_%s_%s_%s_%d.txt"
                    % (self.args.task, mode, self.args.method, global_step),
                ),
                preds,
                self.id2label,
            )
            ...
            write_prediction_wic(
                ...
                os.path.join(
                    self.args.eval_dir,
                    "pred/pred_%s_%s_%s_%s.txt"
                    % (self.args.task, mode, self.args.method, str(global_step)),
                ),
                preds,
                self.id2label,
            )
        ...
        print(
            "Macro F1: %.4f, Micro F1: %.4f, Accu: %.4f"
            % (result["macro-f1"], result["micro-f1"], result["acc"])
        )
        write_f1_tc(
            self.args,
            os.path.join(self.args.eval_dir, "pred_%s_%s" % (self.args.task, mode)),
            result["macro-f1"],
            result["micro-f1"],
            result["acc"],
            global_step,
    ...
    def load_model(self):
        ...
        try:
            ...
            self.model = self.model_class.from_pretrained(
                self.args.model_dir, config=self.bert_config, args=self.args
            )
            self.model.to(self.device)
