...
namespace langdetect
{
    ...
    std::string const Detector::UNKNOWN_LANG = "other";
    ...
    Detected Detector::detect(char const *data, size_t const length)
    {
        ...
        if (grams.empty())
            return Detected(UNKNOWN_LANG, 0.0);
        ...
        for (int i = 0; i < trial_; ++i)
        {
            ...
            for (int j = 0;;)
            {
                ...
                if (j % 5 == 0)
                {
                    if (normalize_(scores) > CONV_THRESHOLD || j >= ITERATION_LIMIT)
                        break;
                }
                j += 1; // count up
            }
    ...
    {
        if (minnum > maxnum)
            throw std::runtime_error("min exceeds maximum");
